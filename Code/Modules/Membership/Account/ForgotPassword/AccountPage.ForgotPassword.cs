...
namespace ECNET.Membership.Pages
{
    ...
    using ECNET.Office.Endpoints;
    using MyRow = ECNET.Office.Entities.QueuedEmailRow;

    public partial class AccountController : Controller
    {
        QueuedEmailController queuedEmail;
        IUnitOfWork uow;
        //public AccountController(QueuedEmailController _queuedEmail, IUnitOfWork _uow)
        //{
        //    queuedEmail = _queuedEmail;
        //    uow = _uow;
        //}
        [HttpGet]
        ...
        public Result<ServiceResponse> ForgotPassword(ForgotPasswordRequest request)
        {
            return this.UseConnection("Default", connection =>
            {
                ...
                ////Sua lai them vao QueuedMail
                //var queued = new QueuedEmailRow();
                //queued.Subject = emailSubject;
                //queued.To = user.Email;
                //queued.Body = emailBody;
                //queued.EmailAccountId = 1;//mặc định
                //queued.PriorityId = 1;
                //queued.CreatedOnUtc = DateTime.UtcNow;
                ////queuedMail.CreateNew();
                //uow = new UnitOfWork(connection);
                //queuedEmail = new QueuedEmailController();
                //queuedEmail.Create(uow, new SaveRequest<MyRow>
                //{
                //    Entity = queued
                //});
